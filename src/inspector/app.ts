...
<script nonce="${nonce}">
  ...
  async function loadTelemetry(){var grid=document.getElementById('telemetryGrid');grid.innerHTML='<p class="muted">Loading telemetry…</p>';try{var results=await Promise.all([api('/v1/metrics/adoption',{windowDays:7},false),api('/v1/metrics/adoption',{windowDays:30},false),api('/v1/metrics/ontology-governance',{projectHint:currentProject,windowDays:30},false)]);grid.innerHTML=telemetryCard(results[0],7,'adoption')+telemetryCard(results[0],7,'writeback')+telemetryCard(results[1],30,'adoption')+telemetryCard(results[1],30,'writeback')+governanceCard(results[2]);}catch(error){grid.innerHTML='<div class="empty">Telemetry unavailable: '+escapeHtml(error.message)+'</div>';}}
