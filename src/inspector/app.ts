...
<div class="shell">
  ...
  <main>
    ...
    <section id="telemetry" class="view">
      <div><div class="eyebrow">Telemetry + ontology contracts</div><h1>Auditable product health</h1><p class="muted">Eligible denominators, exclusions, unobservable work, and ontology governance decisions are reported separately. Legacy records stay labelled history.</p><button id="reloadTelemetry" class="button">Refresh</button><div id="telemetryGrid" class="telemetry-grid"></div></div>
    </section>
  </main>
</div>
...
<script nonce="${nonce}">
  ...
  function sourceCoverageCard(data,windowDays){var metric=data.sourceWindow.eligibility;return '<article class="telemetry-card"><div class="eyebrow">'+windowDays+' day · source coverage v'+metric.contractVersion+'</div><h2>Eligible coverage '+ratio(metric.ratio)+'</h2><div class="stats"><div class="stat"><strong>'+metric.numerator+'/'+metric.eligibleDenominator+'</strong>measured / eligible</div><div class="stat"><strong>'+metric.ineligible+'</strong>ineligible</div><div class="stat"><strong>'+metric.unobservable+'</strong>unobservable</div></div>'+reasonRows(metric.reasons.eligible,'Eligible reasons')+reasonRows(metric.reasons.ineligible,'Ineligible reasons')+reasonRows(metric.reasons.unobservable,'Unobservable reasons')+'<p class="muted">Legacy mixed coverage: '+data.sourceWindow.coveredEvidenceCount+'/'+data.sourceWindow.selectedEvidenceCount+'. Source-window savings remain limited to measured evidence.</p></article>';}
  function governanceCard(data){var entity=data.registry.entityKinds,relation=data.registry.relationTypes,decisions=data.decisions;return '<article class="telemetry-card"><div class="eyebrow">'+data.windowDays+' day · ontology governance v'+data.contractVersion+'</div><h2>Vocabulary decisions</h2><div class="stats"><div class="stat"><strong>'+decisions.accepted+'</strong>accepted</div><div class="stat"><strong>'+decisions.rejected+'</strong>rejected</div><div class="stat"><strong>'+decisions.deprecated+'</strong>deprecated</div></div><div class="eyebrow">Registry</div><div class="reason-list"><div><span>Entity kinds · active / legacy / deprecated</span><strong>'+entity.active+' / '+entity.legacy+' / '+entity.deprecated+'</strong></div><div><span>Relation types · active / legacy / deprecated</span><strong>'+relation.active+' / '+relation.legacy+' / '+relation.deprecated+'</strong></div></div>'+reasonRows(decisions.reasons.accepted,'Accepted reasons')+reasonRows(decisions.reasons.rejected,'Rejected reasons')+reasonRows(decisions.reasons.deprecated,'Deprecated reasons')+'</article>';}
  async function loadTelemetry(){var grid=document.getElementById('telemetryGrid');grid.innerHTML='<p class="muted">Loading telemetry…</p>';try{var results=await Promise.all([api('/v1/metrics/adoption',{windowDays:7},false),api('/v1/metrics/adoption',{windowDays:30},false),api('/v1/metrics/context',{projectHint:currentProject,windowDays:7,typingWordsPerMinute:40},false),api('/v1/metrics/context',{projectHint:currentProject,windowDays:30,typingWordsPerMinute:40},false),api('/v1/metrics/ontology-governance',{projectHint:currentProject,windowDays:30},false)]);grid.innerHTML=telemetryCard(results[0],7,'adoption')+telemetryCard(results[0],7,'writeback')+telemetryCard(results[1],30,'adoption')+telemetryCard(results[1],30,'writeback')+sourceCoverageCard(results[2],7)+sourceCoverageCard(results[3],30)+governanceCard(results[4]);}catch(error){grid.innerHTML='<div class="empty">Telemetry unavailable: '+escapeHtml(error.message)+'</div>';}}
